"""
오프라인 NDCG 검증 파이프라인 (Fast Version)
- 샘플링 기반 빠른 검증
- 10,000명 사용자 샘플
...
def evaluate_model(model, train_sparse, test_df, n_users, k=10, sample_size=10000):
    """모델 평가 (샘플링)"""
...
def run_experiment(train_df, n_users, n_items, als_params,
                   use_weights=False, weight_map=None, test_ratio=0.2, sample_size=10000):
    """단일 실험 실행"""
    # Train/Test 분할
...
def main():
    parser = argparse.ArgumentParser(description='오프라인 NDCG 검증 (Fast)')
    parser.add_argument('--dir_path', type=str, default='../data/')
    parser.add_argument('--data_file', type=str, default='train.parquet')
    parser.add_argument('--test_ratio', type=float, default=0.2)
    parser.add_argument('--sample_size', type=int, default=10000, help='평가 샘플 수')
    ...
    print("=" * 60)
    print("오프라인 NDCG 검증 파이프라인 (Fast - 샘플링)")
    print("=" * 60)
    ...
    # 실험 2: event_type 가중치 적용 (view=1, cart=5, purchase=10)
    print("\n[3/5] 실험 2: event_type 가중치 (view=1, cart=5, purchase=10)")
    ...
    # 실험 3: 더 강한 가중치 (view=1, cart=10, purchase=20)
    print("\n[4/5] 실험 3: 더 강한 가중치 (view=1, cart=10, purchase=20)")
    weight_map_strong = {'view': 1, 'cart': 10, 'purchase': 20}
    metrics = run_experiment(train_df, n_users, n_items, als_params,
                             use_weights=True, weight_map=weight_map_strong,
                             test_ratio=args.test_ratio, sample_size=args.sample_size)
    results.append({
        'experiment': 'ALS (v1,c10,p20)',
        **metrics
    })
    print(f"  NDCG@10: {metrics['ndcg']:.4f}")
    print(f"  Recall@10: {metrics['recall']:.4f}")
    ...
    # 실험 4: 더더 강한 가중치 (view=1, cart=15, purchase=30)
    print("\n[5/5] 실험 4: 매우 강한 가중치 (view=1, cart=15, purchase=30)")
    weight_map_very_strong = {'view': 1, 'cart': 15, 'purchase': 30}
    metrics = run_experiment(train_df, n_users, n_items, als_params,
                             use_weights=True, weight_map=weight_map_very_strong,
                             test_ratio=args.test_ratio, sample_size=args.sample_size)
    results.append({
        'experiment': 'ALS (v1,c15,p30)',
        **metrics
    })
    print(f"  NDCG@10: {metrics['ndcg']:.4f}")
    print(f"  Recall@10: {metrics['recall']:.4f}")
    ...
    # CSV 저장
    results_df.to_csv('offline_validation_results_fast.csv', index=False)
    print("\n결과 저장: offline_validation_results_fast.csv")
